...
export default function LecturerLoginPage() {
  ...
  const { lecturerLogin, loading, error } = useAuth();
  ...
  const handle_submit = async (e: React.FormEvent) => {
    ...
    if ((!staff_id.trim() && !email.trim()) || !password.trim()) {
      set_error_message("Please provide Staff ID or Email and Password.");
      return;
    }
    ...
    try {
      // Build payload — supports both staff_id and email login
      const payload = { staff_id: staff_id || undefined, email: email || undefined, password };

      const data = await lecturerLogin(payload);
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#F9FAFB] p-4">
      <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
        ...
        <form onSubmit={handle_submit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Enter Staff ID (e.g., ST-001)"
        ...
        <footer className="text-center text-xs text-gray-400 mt-6">
          © {new Date().getFullYear()} AFUED Portal — Lecturer Access
        </footer>
      </div>
    </div>
  );
